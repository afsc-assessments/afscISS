...
get_surveyISS <- function(region){
  ...
  # ebs
  if(region == 'ebs'){
    if(length(list.files(here::here('output', region), pattern = "turb_bin")) > 0){
      base_bin <- purrr::map(list.files(here::here('output', region), pattern = "turb_bin_base"), ~ vroom::vroom(here::here('output', region, .)))
      names(base_bin) <- list.files(here::here('output', region), pattern = "turb_bin_base") %>% 
        stringr::str_replace(., '.csv', "")
      base <- c(base, base_bin)
      resamp_bin <- purrr::map(list.files(here::here('output', region), pattern = "turb_bin_resampled"), ~ vroom::vroom(here::here('output', region, .)))
      resamp <- c(resamp, resamp_bin)
      comps_bin <- purrr::map(1:length(base_bin), ~ get_comps(base_bin[[.]], resamp_bin[[.]], ., length(base_bin), region))
      names(comps_bin) <- list.files(here::here('output', region), pattern = "turb_bin_base") %>% 
        stringr::str_replace(., '.csv', "") %>% 
        stringr::str_replace(., 'base', "comp")
  ...
  # goa
  if(region == 'goa'){
    if(length(list.files(here::here('output', region), pattern = "pcod_bin")) > 0){
      base_bin <- purrr::map(list.files(here::here('output', region), pattern = "pcod_bin_base"), ~ vroom::vroom(here::here('output', region, .)))
      names(base_bin) <- list.files(here::here('output', region), pattern = "pcod_bin_base") %>% 
        stringr::str_replace(., '.csv', "")
      base_bin2 <- purrr::map(list.files(here::here('output', region), pattern = "rex_bin_base"), ~ vroom::vroom(here::here('output', region, .)))
      ...
      comps_bin <- purrr::map(1:length(base_bin), ~ get_comps(base_bin[[.]], resamp_bin[[.]], ., length(base_bin), region))
      names(comps_bin) <- list.files(here::here('output', region), pattern = "pcod_bin_base") %>% 
        stringr::str_replace(., '.csv', "") %>% 
        stringr::str_replace(., 'base', "comp")
      comps_bin2 <- purrr::map(1:length(base_bin2), ~ get_comps(base_bin2[[.]], resamp_bin2[[.]], ., length(base_bin2), region))
      names(comps_bin2) <- list.files(here::here('output', region), pattern = "rex_bin_base") %>% 
        stringr::str_replace(., '.csv', "") %>% 
        stringr::str_replace(., 'base', "comp")
      comps <- c(comps, comps_bin, comps_bin2)
    }
  }
  # nebs
  if(region == 'nebs'){
  ...
  if(region == 'ebs'){
    if(length(list.files(here::here('output', region), pattern = "turb_bin")) > 0){
      bias_bin <- purrr::map(list.files(here::here('output', region), pattern = "turb_bin_bias"), ~ vroom::vroom(here::here('output', region, .)))
      names(bias_bin) <- list.files(here::here('output', region), pattern = "turb_bin_bias") %>% 
        stringr::str_replace(., '.csv', "")
  ...
  if(region == 'goa'){
    if(length(list.files(here::here('output', region), pattern = "pcod_bin")) > 0){
      bias_bin <- purrr::map(list.files(here::here('output', region), pattern = "pcod_bin_bias"), ~ vroom::vroom(here::here('output', region, .)))
      names(bias_bin) <- list.files(here::here('output', region), pattern = "pcod_bin_bias") %>% 
        stringr::str_replace(., '.csv', "")
      bias_bin2 <- purrr::map(list.files(here::here('output', region), pattern = "rex_bin_bias"), ~ vroom::vroom(here::here('output', region, .)))
      names(bias_bin2) <- list.files(here::here('output', region), pattern = "rex_bin_bias") %>% 
        stringr::str_replace(., '.csv', "")
      bias <- c(bias, bias_bin, bias_bin2)
    }
  }
  # nebs
  if(region == 'nebs'){
  ...
  # ebs
  if(region == 'ebs'){
    if(length(list.files(here::here('output', region), pattern = "turb_bin_iss")) > 0){
      iss1_bin <- purrr::map(list.files(here::here('output', region), pattern = "turb_bin_iss"), ~ vroom::vroom(here::here('output', region, .)))
      rss_bin <- purrr::map(list.files(here::here('output', region), pattern = "turb_bin_iter_rss"), ~ vroom::vroom(here::here('output', region, .)))
      iss_bin <- purrr::map(1:length(iss1_bin), ~ iss_stats(iss1_bin[[.]], rss_bin[[.]], ., length(iss1_bin), region))
      names(iss_bin) <- list.files(here::here('output', region), pattern = "turb_bin_iss") %>% 
        stringr::str_replace(., '.csv', "")
      iss <- c(iss, iss_bin)
    }
  }
  ...
  # goa
  if(region == 'goa'){
    if(length(list.files(here::here('output', region), pattern = "pcod_bin_iss")) > 0){
      iss1_bin <- purrr::map(list.files(here::here('output', region), pattern = "pcod_bin_iss"), ~ vroom::vroom(here::here('output', region, .)))
      rss_bin <- purrr::map(list.files(here::here('output', region), pattern = "pcod_bin_iter_rss"), ~ vroom::vroom(here::here('output', region, .)))
      iss_bin <- purrr::map(1:length(iss1_bin), ~ iss_stats(iss1_bin[[.]], rss_bin[[.]], ., length(iss1_bin), region))
      names(iss_bin) <- list.files(here::here('output', region), pattern = "pcod_bin_iss") %>% 
        stringr::str_replace(., '.csv', "")
      iss1_bin2 <- purrr::map(list.files(here::here('output', region), pattern = "rex_bin_iss"), ~ vroom::vroom(here::here('output', region, .)))
      rss_bin2 <- purrr::map(list.files(here::here('output', region), pattern = "rex_bin_iter_rss"), ~ vroom::vroom(here::here('output', region, .)))
      iss_bin2 <- purrr::map(1:length(iss1_bin2), ~ iss_stats(iss1_bin2[[.]], rss_bin2[[.]], ., length(iss1_bin2), region))
      names(iss_bin2) <- list.files(here::here('output', region), pattern = "rex_bin_iss") %>% 
        stringr::str_replace(., '.csv', "")
      iss <- c(iss, iss_bin, iss_bin2)
    }
  }
  # nebs
  if(region == 'nebs'){
