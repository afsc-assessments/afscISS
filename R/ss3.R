get_ss3_comp <- function(species, region, comp, sex_cat=12, bysex = FALSE, spec_case=NULL,
                         max_age=12, seas=1, flt=2, part=0, Ageerr=1, Lbin_lo=1, Lbin_hi=-1) {
  if(comp=='caal' & sex_cat!=0) {stop('must set: sex_cat=0 for CAAL')}
  ...
  if(isFALSE(bysex)){
    ...
    # full dataset
    expand.grid(year = years, species_code = unique(data$species_code), sex = sexes,
                age = bin) %>% 
      merge(dat, all=T) %>% 
    ...
    expand.grid(year = years, species_code = unique(data$species_code), sex = sexes,
                age = bin) %>% 
      merge(tidytable::bind_rows(dat, dat2), all=T) %>% 
      tidytable::left_join(tidytable::select(iss, year, iss)) %>% 
      tidytable::replace_na(list(prop = 0)) %>%
      tidytable::mutate(prop = tidytable::case_when(age>=max_age ~ sum(prop),
                                                    TRUE ~ prop),
