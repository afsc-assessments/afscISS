...
2. `afscISS::get_comp()`: obtains the computed age, length, and conditional age-at-length composition from design-based population estimates (i.e., the proportions that would be used in a stock assessment model). The output that will return from the `afscISS::get_comp()` function for `comp = 'age'` and `comp = 'length'` will include the following columns:
...
3. `afscISS::get_bias()`: obtains the average bias in the bootstrap simulated age/length/conditional age-at-length. The output that will return from the `afscISS::get_bias()` function for `comp = 'age'` and `comp = 'length'` will include the following columns:
...
4. `afscISS::get_RSS()`: obtains statistics for bootstrap results for the iterated realized sample size. The output that will return from the `afscISS::get_RSS()` function for `comp = 'age'` and `comp = 'length'` will include the following columns:
...
5. `afscISS::get_bs_comp()`: obtains statistics for bootstrap results for the iterated age/length/conditional age-at-length composition. The output that will return from the `afscISS::get_bs_comp()` function for `comp = 'age'` and `comp = 'length'` will include the following columns:
